...
const LessonViewMode = {
  ...
  apply() {
    const tables = document.querySelectorAll('.lesson-table');
    if (tables.length === 0) { console.log('[LessonViewMode] No tables yet'); return; }
    ...
    tables.forEach(function(table) {
      ...
      // 3 colonnes : French | Phonetics | English
      if (n === 3) {
        if (inverted) {
          ths[0].textContent = 'English'; ths[1].textContent = 'Phonetics'; ths[2].textContent = 'French';
        } else {
          ths[0].textContent = 'French'; ths[1].textContent = 'Phonetics'; ths[2].textContent = 'English';
        }
        for (let r = 1; r < rows.length; r++) {
          const tds = rows[r].querySelectorAll('td');
          if (tds.length < 3) continue;
          if (!rows[r].dataset.original) {
            rows[r].dataset.original = JSON.stringify([tds[0].innerHTML, tds[1].innerHTML, tds[2].innerHTML]);
          }
          const original = JSON.parse(rows[r].dataset.original);
          if (inverted) {
            tds[0].innerHTML = original[2]; tds[1].innerHTML = original[1]; tds[2].innerHTML = original[0];
          } else {
            tds[0].innerHTML = original[0]; tds[1].innerHTML = original[1]; tds[2].innerHTML = original[2];
          }
        }
      }
      // 2 colonnes : French | English
      else if (n === 2) {
        if (inverted) {
          ths[0].textContent = 'English'; ths[1].textContent = 'French';
        } else {
          ths[0].textContent = 'French'; ths[1].textContent = 'English';
        }
        for (let r = 1; r < rows.length; r++) {
          const tds = rows[r].querySelectorAll('td');
          if (tds.length < 2) continue;
          if (!rows[r].dataset.original) {
            rows[r].dataset.original = JSON.stringify([tds[0].innerHTML, tds[1].innerHTML]);
          }
          const original = JSON.parse(rows[r].dataset.original);
          if (inverted) {
            tds[0].innerHTML = original[1]; tds[1].innerHTML = original[0];
          } else {
            tds[0].innerHTML = original[0]; tds[1].innerHTML = original[1];
          }
        }
      }
    });
